...
const ViewedProfiles = () => {
    ...
    const loadViewedProfiles = async () => {
        try {
            const stored = await AsyncStorage.getItem('viewed_profiles_list');
            if (stored) {
                const parsed = JSON.parse(stored);
                // Sort by viewedAt timestamp descending
                parsed.sort((a, b) => new Date(b.viewedAt) - new Date(a.viewedAt));
                setViewedProfiles(parsed);
            }
        } catch (error) {
    ...
    const renderProfileCard = ({ item }) => {
        const name = decodeUTF8String(item.user_name || item.name || item.profile_name) || 'Unknown';
        const imageUrl = item.profile_image || (item.user_photo ? `https://nadarmahamai.com/uploads/${item.user_photo}` : null);
        const profileId = item.id || item.profile_id; // Prefer numeric ID if available

        // Date formatting for "Status Badge"
        const viewedDate = new Date(item.viewedAt);
        const dateString = viewedDate.toLocaleDateString();

        // Data processing for fallback
        const educationLabel = item.education || item.padippu || '';
        const occupationLabel = item.occupation || item.profession || 'Not Specified';
        const locationLabel = item.location || item.city || 'Unknown';
            ...
            <View style={styles.cardWrapper}>
                ...
                >
                    <View style={styles.profileCard}>
                        {/* Header: ID (Left) | Viewed Date (Right) */}
                        <View style={styles.cardHeader}>
                            <View style={styles.profileBadge}>
                                <Text style={styles.profileIdText}>{profileId}</Text>
                            </View>
                            <View style={styles.statusBadge}>
                                <Icon name="clock-outline" size={scale(12)} color="#EF6C00" />
                        ...
                        <View style={styles.cardBody}>
                            ...
                            <View style={styles.avatarContainer}>
                                ...
                                <View style={styles.matchScoreContainer}>
                                    {/* Just showing 'Viewed' badge since this is Viewed Profiles page */}
                                    <View style={styles.verifiedBadgeSmall}>
                                        <Icon name="eye-check-outline" size={scale(11)} color="#2E7D32" />
                                        <Text style={styles.badgeLabelSmall}>Visited</Text>
                                    </View>
                                </View>
                            </View>
    ...
    return (
        <View style={styles.container}>
            ...
            {isLoading ? (
                ...
                <View style={styles.centerContainer}>
                    <Icon name="eye-off-outline" size={48} color="#ccc" />
                    <Text style={styles.emptyText}>No profiles viewed yet</Text>
                </View>
            ) : (
                <FlatList
                    data={viewedProfiles}
